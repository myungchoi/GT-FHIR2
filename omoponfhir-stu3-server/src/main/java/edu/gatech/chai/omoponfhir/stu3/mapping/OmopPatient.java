...
import edu.gatech.chai.omopv5.jpa.service.ConditionOccurrenceService;
...
public class OmopPatient extends BaseOmopResource<Patient, FPerson, FPersonService>
	...
	private DeviceExposureService deviceExposureService;
	...
	private void initialize(WebApplicationContext context) {
		...
		deviceExposureService = context.getBean(DeviceExposureService.class);
		fObservationViewService = context.getBean(FObservationViewService.class);
		...
		procedureOccurrenceService = context.getBean(ProcedureOccurrenceService.class);
			...
			for (Identifier identifier : identifiers) {
				if (identifier.getValue().isEmpty() == false) {
					...
					if (listPatient != null && listPatient.size() > 0) {
						...
						System.out.println("Strange0::::::::::::::: for " + personSourceValue.toLowerCase() + " "
								+ person.getId());
					}
					if (person != null) {
						omopId = person.getId();
